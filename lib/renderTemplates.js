...
function renderStatic (staticDef, viewsPath, distPath, data, knownHelpers) {
    const destPath = getDestPath(staticDef.path || staticDef.view);
    const destFileName = path.join(distPath, destPath);
    const viewsData = Object.assign({}, data, staticDef);

    return ensureDirectories(distPath, destPath)
        .then(() => renderTemplate(viewsPath, staticDef.view, viewsData, knownHelpers))
